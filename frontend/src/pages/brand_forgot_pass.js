...
const ForgotUI = () => {
  ...
  const handleForgotPassword = async () => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      try {
        ...
        setErrors({ forgotGeneral: errorMsg });
      } finally {
        setIsLoading(false);
      }
    }
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen w-full overflow-hidden">
      ...
      {/* Right Side: Slider (Unchanged from previous code) */}
      <div className="hidden md:block w-full md:w-1/2 h-64 md:h-screen bg-gray-900 relative">
        {/* Slider content remains the same as in the original code */}
